...
client.once('ready', () => {
  ...
  checkHours();
  setInterval(checkHours, 300000); // co 5 minut
});
...
async function checkHours() {
  try {
    ...
    if (latest && latest.id !== lastMessageId) {
      ...
      console.log('Nowa lista godzin – zapisuję...');

      const lines = latest.content.split('\n').filter(l => l.includes('–') && l.includes('h'));

      const snapshot = await get(dbRef);
      let officers = snapshot.val() ? Object.values(snapshot.val()) : [];
